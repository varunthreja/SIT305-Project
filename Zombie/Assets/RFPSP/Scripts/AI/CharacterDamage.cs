...
public class CharacterDamage : MonoBehaviour {
	...
	public void ApplyDamage ( float damage, Vector3 attackDir, Vector3 attackerPos, Transform attacker, bool isPlayer, bool isExplosion, Rigidbody hitBody = null, float bodyForce = 0f ){
		...
		if (hitPoints <= 0.0f){
            ...
            //GamePlay.instance.BlockyDeathSound.Play();
            //transform.GetChild(0).gameObject.SetActive(true);
            //transform.GetChild(1).transform.GetChild(1).gameObject.SetActive(false);
            transform.GetComponent<bl_MiniMapItem>().DestroyItem(true);
            GamePlay.instance.TargetKills--;
            GamePlay.instance.EnemiesToKillText.text = GamePlay.instance.TargetKills.ToString();
            GamePlay.instance.TotalKills++;
            GamePlay.instance.CheckLevelComplete();
        }
	}
