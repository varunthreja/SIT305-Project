...
public class FPSPlayer : MonoBehaviour {
    ...
    public Text MyHealthText , MyHungerText, MyAmmoText;
...
//		MainMenuComponent.enabled = false;
		...
		healthGuiObjInstance = Instantiate(healthGuiObj, Vector2.zero ,transform.rotation) as GameObject;
		if(showHelpText){
        ...
        MyHealthText.text = "Health : " + hitPoints.ToString();
		...
		if(usePlayerHunger){
            ...
            MyHungerText.text = "Hunger : " + hungerPoints.ToString();
		}
					...
					if(pickUpBtnState){//check pickUpBtnState to prevent reticle from briefly showing custom/general pickup icon briefly when picking up last weapon before maxWeapons are obtained
						...
						//determine if item under reticle is a health pickup	
							...
							if(HealthPickupComponent.healthPickupReticle){
                                ...
                                //GamePlay.instance.PickUpButton.SetActive(true);
                                //PickUpButton.SetActive(true);
                            }
                            else{
								pickupTex = pickupReticle;
...
//	}
	...
	public void HealPlayer( float healAmt, bool isHungryThirsty = false ){
        ...
        //MyHealthText.text = Mathf.Round(hitPoints);
	...
	void Die () {
        ...
        GamePlay.instance.LevelFailMethod();
	}
