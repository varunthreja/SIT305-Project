	...
	void Update(){
		...
		if(ammoGui != oldAmmo || ammoGui2 != oldAmmo2 || oldWidth != Screen.width) {
			...
			if(showMags){
                ...
                FPSPlayer.instance.MyAmmoText.text = ammoGui.ToString() + " / " + ammoGui2.ToString();
            }
            else{
                //guiTextComponent.text = "Ammo : "+ ammoGui2.ToString();
                FPSPlayer.instance.MyAmmoText.text = ammoGui2.ToString();
        ...
        if(ammoGui == 0 && ammoGui2 == 0)
        {
            GamePlay.instance.ReloadAdButton.SetActive(true);
        }
