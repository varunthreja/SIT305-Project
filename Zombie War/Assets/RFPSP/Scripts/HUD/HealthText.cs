	...
	void Update (){
		...
		if(healthGui != oldHealthGui || oldWidth != Screen.width){
			if(healthGui < 0.0f && !showNegativeHP){
                ...
                FPSPlayer.instance.MyHealthText.text = "0";

            }
            else{
				//guiTextComponent.text = "Health : " + healthGui.ToString();
                FPSPlayer.instance.MyHealthText.text = healthGui.ToString();
            }
        ...
        if(healthGui < 10 && !CallButton)
        {
            CallButton = true;
            FPSPlayer.instance.MyHealthText.text = "00";
            if (count == 0)
            {
                GamePlay.instance.SaveMeAdMethod();
                count++;
            }
            else
            {
